...
export class FlipCalculation2 {
  ...
  public getFlippingProgress(): number {
    return Math.abs(
      ((Helper.handleNull(this.position).x - this.pageWidth) /
        (2 * this.pageWidth)) *
        100
    );
  }
  ...
  private calcAngleAndPosition(pos: Point): Point {
    ...
    if (
      Math.abs(Helper.handleNull(result).x - this.pageWidth) < 1 &&
      Math.abs(Helper.handleNull(result).y) < 1
    ) {
  ...
  private calculateAngle(pos: Point): number {
    const posX = Helper.handleNull(pos).x;
    const posY = Helper.handleNull(pos).y;
    const left = this.pageWidth - posX + 1;
    const top =
      this.corner === FlipCorner.BOTTOM ? this.pageHeight - posY : posY;
  ...
  private getRotatedPoint(transformedPoint: Point, startPoint: Point): Point {
    ...
    return {
      x:
        adjustedTransformedPoint.x * Math.cos(this.angle) +
        adjustedTransformedPoint.y * Math.sin(this.angle) +
        adjustedStartPoint.x,
      y:
        adjustedTransformedPoint.y * Math.cos(this.angle) -
        adjustedTransformedPoint.x * Math.sin(this.angle) +
  ...
  private checkPositionAtCenterLine(
    ...
    if (Helper.handleNull(checkPointOne).x <= 0) {
...
const FlipCalculation = { getFlippingProgress, getAngle };
